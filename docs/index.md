...
## Audio samples generated by different TTS models
We trained VITS and FastPitch model using the recipe and code provided in [Coqui-TTS][coqui_tts_link] with the same batchsize on a single GPU for 100k iterations. VITS learned to speak with Word Rrror Rate (WER) of 4.4% while FastPitch did not learn to speak at all, it started uttering some speech after 140k iterations but it was not intelligible and had a WER of 20%.
> Note: OverFlow during this time during the training had a WER of ~3.2% and had only received 30k gradient updates.
